    ...
    $.fn.extend({ 
        ...
        genMCQuestion: function(options) {
            ...
            return this.each(function() {
				...
				function clickCfmBtn(thisObj) {
					var qParent =$(thisObj).parent("div").parent("li");
					...
					if (rightAns=="Y") {
						...
						if (noOfQuest == $("input[value='Y']:checked", obj).size()) {
							$(obj).children("li").css("background-color","rgba(0,0,255, 0.1)");
							var btn = $("p.cfmButton", obj);
							
							$(obj).off("click","li > div > p.cfmButton");
						};
						
						//$("p.cfmButton").css("opacity", "0"):									
				...
				$(ctrlObj).on("click", "p#showall", function() {
					...
					$.each($(ansList), function() {
						...
						//$(this).parent("li").parent("ul").parent("li").children("div").children("p.cfmButton").hide();
						//$(this).parent("li").parent("ul").parent("li").children("div").children("p.cfmButton").off("click");
						$(obj).off("click","li > div > p.cfmButton");
				...
				$(ctrlObj).on("click", "p#resetall", function() {
					...
					$.each($(ansList), function() {
						...
						//$(this).parent("li").parent("ul").parent("li").children("div").children("p.cfmButton").show();
						//$(this).parent("li").parent("ul").parent("li").children("div").children("p.cfmButton").on("click");
						$(obj).on("click","li > div > p.cfmButton", function() {
							clickCfmBtn($(this));
						});
					});
